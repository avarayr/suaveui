...
const input = cva("input", {
  variants: {
    intent: {
      ...
      secondary: [
        "bg-input",
        "[&:has(input:focus)]:brightness-105",
        "[&_input]:placeholder-[#777777]",
      ],
    },
  },
  defaultVariants: {
...
type GenerativeOpts = {
  prompt:
    | string
    | ((value: React.ComponentPropsWithoutRef<"input">["value"]) => string);
  system?: string;
  max_tokens: number;
  temperature?: number;
};
  ...
  (props, ref) => {
    ...
    const onGenerateClick = async () => {
      ...
      if (typeof props.generative.prompt === "function") {
        ...
        generativePrompt = props.generative.prompt.replace(
          "{{value}}",
          String(props.value),
        );
      }
    ...
    return (
      <div ref={ref} className="w-full">
        {props.label && (
          <label className="mb-2 pl-[0.1rem] text-xs font-medium text-[#8F8F95]">
            {props.label}
          </label>
        )}
        <div
